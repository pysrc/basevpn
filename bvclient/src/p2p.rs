...
pub async fn get_peer_addr(cfg: RelayConfig) -> Option<(SocketAddr, SocketAddr)> {
    ...
    // 获取对方公网地址
    socket.send_to(&buffer, relay).await.unwrap();
    let peer_addr = match socket.recv_from(&mut recv_buf).await {
        Ok((n, _)) => {
            let _back = String::from_utf8_lossy(&recv_buf[..n]).to_string();
            if _back.starts_with("NOPEER") {
                log::info!("inside not login.");
                return None;
            } else {
                match SocketAddr::from_str(_back.as_str()) {
                    Ok(_addr) => {
                        _addr
                    }
                    Err(e) => {
    ...
    if success {
        ...
        // 打洞失败申请中继
        log::info!("FOR RELAY");
        let mut relay_buffer = [0u8; 255 + 41];
